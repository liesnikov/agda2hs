...
simpleFun x
  | x > 0 = RuntimeCheckFeatures.PostRtc.simpleFun x
...
tripleOdd a0
  ...
  where
    go1 up m a2
      | m > 0 = up m (\ a3 -> a2 (go0 a3))
      | otherwise = error "Runtime check failed: m > 0"
      where
        go0 up n
          | not (n < 1) = up n
          | otherwise = error "Runtime check failed: not (n < 1)"
...
scRec recFst recSnd = Rec recFst (go0 recSnd)
  where
    go0 up
      | recFst > 0 = up
      | otherwise = error "Runtime check failed: recFst > 0"
...
scErasedField erasedFst
  | erasedFst > 0 = ErasedField erasedFst
  | otherwise = error "Runtime check failed: erasedFst > 0"
